...
object ScalrAssets extends Controller {
   ...
   */
  def at(fileuid: String, width: Int, height: Int, mode: Scalr.Mode = Scalr.Mode.AUTOMATIC, source: String = "default")(implicit app: Application): Action[AnyContent] = Action { request =>
    ...
    val scalr = app.plugin(classOf[ScalrPlugin]).get
    scalr.get(fileuid, source, width, height, Scalr.Method.ULTRA_QUALITY, mode).map { resizedImage =>
